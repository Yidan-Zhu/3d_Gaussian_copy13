...
#	if event.position.x < margin_right_viewport:
	if event.position.x < 766 and event.position.y > 100:  # avoid having no intersecting point with (0,1,0) plane
				...
				if draw2.y >= reference_y_2 - contour_back_bending_index && draw1.y >= reference_y_1 - contour_back_bending_index:
					...
					for m in range(1,number_of_contour+1):
						...
						if m!=2:
							...
							draw_line(camera.unproject_position(origin+ position_3d_1 + Vector3(0,each_contour_height,0)), camera.unproject_position(origin -position_3d_1+ Vector3(0,each_contour_height,0)), Color(0.91,0.73,0.75,1), 1.0, true)			
							draw_line(camera.unproject_position(origin+ position_3d_2 + Vector3(0,each_contour_height,0)), camera.unproject_position(origin -position_3d_2+ Vector3(0,each_contour_height,0)), Color(0.91,0.73,0.75,1), 1.0, true)
							if !get_node_or_null("Label_a"):
								var node = Label.new()
								node.name = "Label_a"
								add_child(node)	
